...
public class ChengguanEventAttachRunnable implements OuterJobRunnableTemplate {
    ...
    public ChengguanEventAttachRunnable() {
    ...
    private void insertChengguanEventAttach() {
        //1.查询没有附件的事件对应的system_id和EVENT_CODE(筛选条件：HAS_ATTACHMENT为空)
        ...
        DsOuterJobModel outerJobModel = outerUrlsService.getModelByTableName("BLK_CG_EVT_ATTS");
        Integer token_link_id = outerJobModel.getToken_link_id();
        DsOuterJobModel tokenModel = outerUrlsService.getModelByObjectId(token_link_id);
            ...
            if (result != null && result.size() != 0) {
                for (HashMap map : result) {
                    ...
                    if (blk_cg_evt_atts.get(0) == CommonInstance.SUCCESS) {
                        //5.更新chengguan_event表中的HAS_ATTACHMENT，如果步骤3中解析结果为空，则插入0，不为空则插入1
