...
import com.cetc.cloud.datasynch.provider.template.impl.SanxiaoCalcRunnable;
import com.cetc.cloud.datasynch.provider.template.impl.XinfangGetRunnable;
...
public class SingleJobController implements SingleJobRemoteService {
    ...
    public void truncateAndReSynchTable(String tableName) {
        ...
        jobManageService.startOnceJob(dsScheduleModel);
    }

    @Override
    public void calculateHasTroubleSanXiao() throws SQLException {
        SanxiaoCalcRunnable sanxiaoCalcRunnable = new SanxiaoCalcRunnable();
...
//    @Scheduled(cron = "00 40 08 * * ?")
    ...
    public void insertXinfangDataToday() throws SQLException {
        ...
        XinfangGetRunnable xinfangGetRunnable = new XinfangGetRunnable();
    ...
    public void insertXinfangHistoryData(String min, String max) throws SQLException {
        ...
        //1.获取Http请求信息
        DsOuterJobModel xinfangHistoryModel = outerUrlsService.getModelByTableName("WEEKLY_XINFANG_HISTORY");
        //2.获取关联token请求信息
        DsOuterJobModel tokenModel = outerUrlsService.getModelByObjectId(xinfangHistoryModel.getToken_link_id());
        //3.获取最新token值
        ...
        if (tokenString != null) {
            ...
            JSONObject token = HttpClientUtil2.parseTokenStr2JSONObj(tokenStr);
