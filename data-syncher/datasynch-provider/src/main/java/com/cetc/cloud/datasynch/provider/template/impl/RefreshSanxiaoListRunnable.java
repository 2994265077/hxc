...
public class RefreshSanxiaoListRunnable implements OuterJobRunnableTemplate {
    ...
    public RefreshSanxiaoListRunnable() {
        this.dbThirdOperateService = (DbThirdOperateService) SpringContextUtil.getBean("dbThirdOperateService");
        this.dbOperateService = (DbOperateService) SpringContextUtil.getBean("dbOperateService");
    }

    @Override
    public void run() {
        Thread.currentThread().setName("RefreshSanxiaoListRunnable");
        //1.通过数据库读取三小场所数据_new
//        String sql = "select ID,STATUS from T_KZ_SMALL_PLACE";
    ...
    private List<String> getSanxiaoStatusBatchUpdateSQL(List<SanxiaoModel> modelList) {
        ...
        while (iterator.hasNext()) {
            ...
            String formatedTime = DateFormatUtils.format(new Date(), CommonInstance.DATE_TIME_STANDARD_FORMAT);
            String sql = "update BLK_SANXIAO_PLACE set "
                    + "STATUS='" + next.getStatus() + "',\n"
                    + "LDDM='" + next.getLddm() + "',\n"
                    + "YHCOUNT='" + next.getYhCount() + "'\n"
                    + "YJJC_UPDATE_TIME='" + formatedTime + "'\n"
                    + " where id = '" + next.getId() + "'";
            sqlList.add(sql);
        }
        return sqlList;
