...
//    private Properties tableSeqMapping;
    ...
    public String exactSequenceByTbName(String tableName) throws IOException, SQLException, InterruptedException {
        ...
        if (exists == false) {
            ...
            if (-1 != nextSeqVal) {
                 ...
                 */
                String sql = "alter sequence " + sequenceName + " increment by " + subResult;
                dbOperateService.oracleExecuteSql(sql);
                ...
                dbOperateService.oracleUpdateSql("alter sequence " + sequenceName + " increment by 1");
    ...
    private void exactSequenceBySEQName(String seqName) {
    ...
    public String createSequence(String sequenceName){
        JSONObject res = new JSONObject();
        if (false==dbOperateService.createSequence(sequenceName)){
            log.error("sequenceName:"+sequenceName+" is illegal");
            res.put("res","fail");
            res.put("msg","sequenceName:"+sequenceName+" is illegal");
            return res.toJSONString();
        }else {
            res.put("res","success");
            res.put("msg","create sequenceName:"+sequenceName+" successful! ");
            return res.toJSONString();
        }
    }
}
