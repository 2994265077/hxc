...
public interface CommentManageRemoteService {
    ...
    List<DsTableCommentModel> importTableCommentExcel(MultipartFile file, String sheetName);
    ...
    //通过表名获取该表字段、字段注释、字段类型
    @RequestMapping(value = "/commentmanage/get/tableAndColumnComment", produces = "application/json", method = RequestMethod.POST)
    @ApiOperation(value = "批量导入带有注释的Excel文档-字段注释", notes = "", produces = "application/json")
    @ApiImplicitParams({
            @ApiImplicitParam(name = "tableName", value = "表名", required = true, dataType = "String", paramType = "query"),
    })
    String getTableAndColCommentByTableName(String tableName);
