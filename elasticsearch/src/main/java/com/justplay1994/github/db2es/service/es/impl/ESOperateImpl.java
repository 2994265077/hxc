...
import com.justplay1994.github.db2es.plugins.LoadMapPlugins;
...
public class ESOperateImpl implements ESOperate {
    ...
    private String colAlias(String tbName, String col, List<HashMap> list){
        for (HashMap map: list){
            ...
            //String tag = (String) map.get("CATEGORY");
            if (tbNameC.equalsIgnoreCase(tbName)) {
               ...
               /* if ((!StringUtils.isEmpty(tag)) && tag.equalsIgnoreCase(col))
                    return "CATEGORY";*/
            }
        }
        return col;
...
//                        new MyURLConnection().request(db2esConfig.getEsUrl() + url, "DELETE", "");
    ...
    public void esBulkGenerator() {
        class Table2esBulk implements Runnable {
                ...
                while (true) {
                    ...
                    try {
                        ...
                        try {
                            ...
                            if (bulkGeneratorByRow(row,tableNode.getTableName()))
             ...
             */
            private boolean bulkGeneratorByRow(List<String> row,String tableName) {
                ...
                try {
                    ...
                    if (isValidGeo(lat, lon)) {
                        ...
                        /*构建body*/
                        map.put("TABLE_NAME",tableName.toUpperCase());
                        if(loadMapPlugins.hasTableNameType.get(tableName.toUpperCase()).equals("1")){
                            map.put("CATEGORY", LoadMyUtil.getPropertiesVauleOfKey("loadmap.properties",tableName+"@"+map.get("LOAD_MAP_TAG_LV2")));
                        }else {
                            map.put("CATEGORY", LoadMyUtil.getPropertiesVauleOfKey("loadmap.properties", tableName.toUpperCase()));
                        }
